...
with open(data_file,'r') as infile:
    while k<11:
        if k == 4 or k == 5 or k == 6 or k == 7 or k == 8 or k == 9:
            k+=1
            gen = islice(infile,N)
            raw_data = np.genfromtxt(gen,delimiter=",", dtype="|S10")
            #print raw_data
            if raw_data.shape[0]<N:
                break
            continue
        gen = islice(infile,N)
        raw_data = np.genfromtxt(gen,delimiter=",", dtype="|S10")
    ...
    #raw_data = np.genfromtxt(data_file,dtype="|S10",delimiter=',')
        ...
        attack_size_data =normal_size_data*(float(100)/float(99)) - normal_size_data
